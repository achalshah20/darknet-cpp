                     ...
                     int log)
{
  ...
  *(l.input_layer) = make_connected_layer(
      batch * steps, inputs, hidden, activation, batch_normalize);
  ...
  *(l.output_layer) = make_connected_layer(
      batch * steps, hidden, outputs, activation, batch_normalize);
...
void backward_rnn_layer(layer l, network_state state)
{
  ...
  for (i = l.steps - 1; i >= 0; --i)
  {
    ...
    /*
       ...
       l.state, 1);
       axpy_cpu(l.hidden * l.batch, 1, self_layer.output - l.hidden*l.batch, 1,
       l.state, 1);
       }else{
       fill_cpu(l.hidden * l.batch, 0, l.state, 1);
       }
...
void forward_rnn_layer_gpu(layer l, network_state state)
{
  ...
  for (i = 0; i < l.steps; ++i)
  {
    ...
    axpy_ongpu(
        l.hidden * l.batch, 1, input_layer.output_gpu, 1, l.state_gpu, 1);
...
void backward_rnn_layer_gpu(layer l, network_state state)
{
  ...
  for (i = l.steps - 1; i >= 0; --i)
  {
    ...
    copy_ongpu(
        l.hidden * l.batch, self_layer.delta_gpu, 1, input_layer.delta_gpu, 1);
