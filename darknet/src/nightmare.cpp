...
// ./darknet nightmare cfg/extractor.recon.cfg ~/trained/yolo-coco.conv
// frame6.png -reconstruct -iters 500 -i 3 -lambda .1 -rate .01 -smooth 2
        ...
        for (jj = j - num; jj <= j + num && jj < recon.h; ++jj)
        {
          ...
          for (ii = i - num; ii <= i + num && ii < recon.w; ++ii)
          {
            ...
            update.data[out_index] +=
                lambda * (recon.data[in_index] - recon.data[out_index]);
          }
        }
      }
    }
...
void run_nightmare(int argc, char **argv)
{
  ...
  if (reconstruct)
  {
    ...
    image crop =
        crop_image(out_im, zz, zz, out_im.w - 2 * zz, out_im.h - 2 * zz);
    // flip_image(crop);
  ...
  for (e = 0; e < rounds; ++e)
  {
    ...
    for (n = 0; n < iters; ++n)
    {
      ...
      {
        reconstruct_picture(
            net, features, im, update, rate, momentum, lambda, smooth_size, 1);
        // if ((n+1)%30 == 0) rate *= .5;
      ...
      else
      {
        ...
        optimize_picture(
            &net, im, layer, 1 / pow(1.33333333, octave), rate, thresh, norm);
      }
    }
    fprintf(stderr, "done\n");
    if (0)
