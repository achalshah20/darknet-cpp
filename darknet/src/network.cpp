...
float get_current_rate(network net)
{
  ...
  switch (net.policy)
  {
    case CONSTANT: return net.learning_rate;
    case STEP: return net.learning_rate * pow(net.scale, batch_num / net.step);
    case STEPS:
      ...
      for (i = 0; i < net.num_steps; ++i)
      {
        ...
        rate *= net.scales[i];
        // if(net.steps[i] > batch_num - 1 && net.scales[i] > 1)
        // reset_momentum(net);
      }
      return rate;
    case EXP: return net.learning_rate * pow(net.gamma, batch_num);
    case POLY:
    ...
    case RANDOM: return net.learning_rate * pow(rand_uniform(0, 1), net.power);
    case SIG:
      return net.learning_rate *
             (1. / (1. + exp(net.gamma * (batch_num - net.step))));
    default: fprintf(stderr, "Policy is weird!\n"); return net.learning_rate;
  }
}
...
matrix network_predict_data_multi(network net, data test, int n)
{
  ...
  for (i = 0; i < test.X.rows; i += net.batch)
  {
    for (b = 0; b < net.batch; ++b)
    {
      ...
      memcpy(
          X + b * test.X.cols, test.X.vals[i + b], test.X.cols * sizeof(float));
    }
    for (m = 0; m < n; ++m)
    {
