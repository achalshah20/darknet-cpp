    ...
    layer l, int batch, float learning_rate, float momentum, float decay)
{
  update_connected_layer(
      *(l.input_layer), batch, learning_rate, momentum, decay);
  update_connected_layer(
      *(l.self_layer), batch, learning_rate, momentum, decay);
  update_connected_layer(
      *(l.output_layer), batch, learning_rate, momentum, decay);
}
    ...
    layer l, int batch, float learning_rate, float momentum, float decay)
{
  update_connected_layer_gpu(
      *(l.input_r_layer), batch, learning_rate, momentum, decay);
  update_connected_layer_gpu(
      *(l.input_z_layer), batch, learning_rate, momentum, decay);
  update_connected_layer_gpu(
      *(l.input_h_layer), batch, learning_rate, momentum, decay);
  update_connected_layer_gpu(
      *(l.state_r_layer), batch, learning_rate, momentum, decay);
  update_connected_layer_gpu(
      *(l.state_z_layer), batch, learning_rate, momentum, decay);
  update_connected_layer_gpu(
      *(l.state_h_layer), batch, learning_rate, momentum, decay);
}
...
void forward_gru_layer_gpu(layer l, network_state state)
{
  ...
  for (i = 0; i < l.steps; ++i)
  {
    ...
    weighted_sum_gpu(
        l.state_gpu, l.h_gpu, l.z_gpu, l.outputs * l.batch, l.output_gpu);
...
void backward_gru_layer_gpu(layer l, network_state state)
{
  ...
  for (i = l.steps - 1; i >= 0; --i)
  {
    ...
    gradient_array_ongpu(
        l.h_gpu, l.outputs * l.batch, TANH, input_h_layer.delta_gpu);
#else
    gradient_array_ongpu(
        l.h_gpu, l.outputs * l.batch, LOGISTIC, input_h_layer.delta_gpu);
#endif

    copy_ongpu(l.outputs * l.batch,
               input_h_layer.delta_gpu,
               1,
               state_h_layer.delta_gpu,
               1);
    ...
    if (prev_delta_gpu)
      mult_add_into_gpu(
          l.outputs * l.batch, l.forgot_delta_gpu, l.r_gpu, prev_delta_gpu);
    ...
    gradient_array_ongpu(
        l.r_gpu, l.outputs * l.batch, LOGISTIC, input_r_layer.delta_gpu);
    copy_ongpu(l.outputs * l.batch,
               input_r_layer.delta_gpu,
               1,
               state_r_layer.delta_gpu,
               1);

    gradient_array_ongpu(
        l.z_gpu, l.outputs * l.batch, LOGISTIC, input_z_layer.delta_gpu);
    copy_ongpu(l.outputs * l.batch,
               input_z_layer.delta_gpu,
               1,
               state_z_layer.delta_gpu,
               1);
