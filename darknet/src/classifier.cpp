...
/*
   void train_classifier(char *datacfg, char *cfgfile, char *weightfile, int
clear)
   {
   srand(time(0));
   float avg_loss = -1;
                    ...
                    int layer_num)
{
  ...
  while (1)
  {
    ...
    image im   = crop_image(
        r, (r.w - 224 - 1) / 2 + 1, (r.h - 224 - 1) / 2 + 1, 224, 224);
    ...
    for (i = 0; i < top; ++i)
    {
      ...
      if (net.hierarchy)
        printf("%d, %s: %f, parent: %s \n",
               ...
               names[index],
               predictions[index],
               (net.hierarchy->parent[index] >= 0)
                   ? names[net.hierarchy->parent[index]]
                   : "Root");
      else
        printf("%s: %f\n", names[index], predictions[index]);
    }
                       ...
                       const char *filename)
{
  ...
  while (1)
  {
    ...
    {
      curr_threat = predictions[218] + predictions[539] + predictions[540] +
                    predictions[368] + predictions[369] + predictions[370];
    }
...
{
#ifdef OPENCV
  int bad_cats[] = {218,   539,   540,   1213,  1501,  1742,  1911,  2415,
                    4348,  19223, 368,   369,   370,   1133,  1200,  1306,
                    2122,  2301,  2537,  2823,  3179,  3596,  3639,  4489,
                    5107,  5140,  5289,  6240,  6631,  6762,  7048,  7171,
                    7969,  7984,  7989,  8824,  8927,  9915,  10270, 10448,
