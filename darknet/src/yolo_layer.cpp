...
layer make_yolo_layer(
    int batch, int w, int h, int n, int total, int *mask, int classes)
{
  int i;
  layer l;
  l.type = YOLO;
                      ...
                      float *avg_cat)
{
  ...
  if (delta[index])
  {
    ...
    if (avg_cat) *avg_cat += output[index + stride * class_];
    return;
  }
  for (n = 0; n < classes; ++n)
  {
    ...
    if (n == class_ && avg_cat) *avg_cat += output[index + stride * n];
  }
}
...
void forward_yolo_layer(const layer l, network net)
{
    ...
    for (j = 0; j < l.h; ++j)
    {
      for (i = 0; i < l.w; ++i)
      {
        for (n = 0; n < l.n; ++n)
        {
          ...
          box pred       = get_yolo_box(l.output,
                                  l.biases,
                                  l.mask[n],
                                  box_index,
                                  i,
          ...
          if (best_iou > l.truth_thresh)
          {
            ...
            int class_ = net.truth[best_t * (4 + 1) + b * l.truths + 4];
            if (l.map) class_ = l.map[class_];
            int class_index =
                entry_index(l, b, n * l.w * l.h + j * l.w + i, 4 + 1);
            delta_yolo_class(l.output,
                             l.delta,
                             class_index,
                             class_,
                             l.classes,
  ...
  *(l.cost) = pow(mag_array(l.delta, l.outputs * l.batch), 2);
  printf(
      "Region %d Avg IOU: %f, Class: %f, Obj: %f, No Obj: %f, .5R: %f, .75R: "
      "%f,  count: %d\n",
      net.index,
      avg_iou / count,
      avg_cat / class_count,
