                        ...
                        int tag)
{
  ...
  if (hier)
  {
    ...
    while (class_ >= 0)
    {
      ...
      delta[index + stride * class_] =
          scale * (1 - output[index + stride * class_]);
  ...
  else
  {
    ...
    for (n = 0; n < classes; ++n)
    {
      delta[index + stride * n] =
          scale * (((n == class_) ? 1 : 0) - output[index + stride * n]);
      ...
      for (i = 0; i < l.w; ++i)
      {
        for (n = 0; n < l.n; ++n)
        {
          ...
          box pred      = get_region_box(
              l.output, l.biases, n, box_index, i, j, l.w, l.h, l.w * l.h);
          float best_iou = 0;
          for (t = 0; t < 30; ++t)
          {
            box truth =
                           ...
                           detection *dets)
{
  ...
  for (i = 0; i < l.w * l.h; ++i)
  {
    ...
    for (n = 0; n < l.n; ++n)
    {
      ...
      dets[index].bbox = get_region_box(
          predictions, l.biases, n, box_index, col, row, l.w, l.h, l.w * l.h);
      ...
      if (l.softmax_tree)
      {
        hierarchy_predictions(
            predictions + class_index, l.classes, l.softmax_tree, 0, l.w * l.h);
        if (map)
        {
          for (j = 0; j < 200; ++j)
          {
  ...
  for (b = 0; b < l.batch; ++b)
  {
    for (n = 0; n < l.n; ++n)
    {
      ...
      if (l.coords > 4)
      {
        ...
        activate_array_gpu(
            l.output_gpu + index, (l.coords - 4) * l.w * l.h, LOGISTIC);
      }
      index = entry_index(l, b, n * l.w * l.h, l.coords);
      if (!l.background)
      ...
      if (!l.softmax && !l.softmax_tree)
        activate_array_gpu(
            l.output_gpu + index, l.classes * l.w * l.h, LOGISTIC);
    }
  }
  if (l.softmax_tree)
  {
...
void backward_region_layer_gpu(const layer l, network net)
{
  ...
  for (b = 0; b < l.batch; ++b)
  {
    for (n = 0; n < l.n; ++n)
    {
      int index = entry_index(l, b, n * l.w * l.h, 0);
      gradient_array_gpu(
          l.output_gpu + index, 2 * l.w * l.h, LOGISTIC, l.delta_gpu + index);
      if (l.coords > 4)
      {
        index = entry_index(l, b, n * l.w * l.h, 4);
      ...
      if (!l.background)
        gradient_array_gpu(
            l.output_gpu + index, l.w * l.h, LOGISTIC, l.delta_gpu + index);
    }
  }
