      ...
      for (t = 0; t < 30; ++t)
      {
        ...
        if (truth.x > 100000 && truth.y > 100000)
        {
          for (n = 0; n < l.n * l.w * l.h; ++n)
          {
            ...
            l.delta[index - 1] =
                l.noobject_scale * ((0 - l.output[index - 1]) *
                                    logistic_gradient(l.output[index - 1]));
            float p = scale * get_hierarchy_probability(
                                  l.output + index, l.softmax_tree, class_);
            if (p > maxp)
            {
              maxp = p;
              maxi = n;
          ...
          if (l.output[index - 1] < .3)
            l.delta[index - 1] =
                l.object_scale * ((.3 - l.output[index - 1]) *
                                  logistic_gradient(l.output[index - 1]));
          else
            l.delta[index - 1] = 0;
          ++class_count;
    ...
    for (j = 0; j < l.h; ++j)
    {
      for (i = 0; i < l.w; ++i)
      {
        for (n = 0; n < l.n; ++n)
        {
          ...
          box pred =
              get_region_box(l.output, l.biases, n, index, i, j, l.w, l.h);
          float best_iou = 0;
          for (t = 0; t < 30; ++t)
          {
            box truth = float_to_box(state.truth + t * 5 + b * l.truths);
          ...
          l.delta[index + 4] =
              l.noobject_scale * ((0 - l.output[index + 4]) *
    ...
    for (n = 0; n < l.n; ++n)
    {
      ...
      if (l.softmax_tree)
      {
        hierarchy_predictions(
            predictions + class_index, l.classes, l.softmax_tree, 0);
        if (map)
        {
          for (j = 0; j < 200; ++j)
          {
        ...
        {
          int j = hierarchy_top_prediction(
              predictions + class_index, l.softmax_tree, tree_thresh);
...
void forward_region_layer_gpu(const layer l, network_state state)
{
   ...
   */
  flatten_ongpu(state.input,
                l.h * l.w,
                l.n * (l.coords + l.classes + 1),
                l.batch,
                1,
                l.output_gpu);
  if (l.softmax_tree)
  {
    ...
    for (i = 0; i < l.softmax_tree->groups; ++i)
    {
      ...
      softmax_gpu(l.output_gpu + count,
                  group_size,
                  l.classes + 5,
                  l.w * l.h * l.n * l.batch,
                  1,
