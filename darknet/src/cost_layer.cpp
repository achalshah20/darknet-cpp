...
void forward_cost_layer_gpu(cost_layer l, network net)
{
  ...
  if (l.cost_type == SMOOTH)
  {
    smooth_l1_gpu(l.batch * l.inputs,
                  net.input_gpu,
                  net.truth_gpu,
                  l.delta_gpu,
                  l.output_gpu);
  ...
  {
    scale_mask_gpu(
        l.batch * l.inputs, l.delta_gpu, 0, net.truth_gpu, l.noobject_scale);
    scale_mask_gpu(
        l.batch * l.inputs, l.output_gpu, 0, net.truth_gpu, l.noobject_scale);
  }
  if (l.cost_type == MASKED)
  {
