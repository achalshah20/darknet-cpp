...
extern "C"
{
  ...
  void normalize_delta_cpu(float *x,
                           ...
                           int spatial,
                           float *delta);
  void l2normalize_cpu(
      float *x, float *dx, int batch, int filters, int spatial);
...
#include "tree.h"
  ...
  void scale_mask_gpu(
      int N, float *X, float mask_num, float *mask, float scale);
  ...
  void flatten_gpu(
      float *x, int spatial, int layers, int batch, int forward, float *out);
